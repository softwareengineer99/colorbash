...
class ColoredText:
	...
	def __init__( self, string, color=None, background=None ):
		self.__setString( string )
		self.set( color, background )
	...
	def set( self, color=None, background=None, key=None ):
		self.setColor( color, key )
		self.setBackground( background, key )

	def setColor( self, color, slice=None ):
		if self.__isNotSet( color ): return
		if self.__isColor( color ):
			slice = self.__slice( slice )
			self.__addColor( slice, color=color )
		else: raise ColorError( 'Color \'%s\' you can\'t use!' % color )

	def setBackground( self, background, slice=None ):
		if self.__isNotSet( background ): return
		if self.__isBackground( background ):
			slice = self.__slice( slice )
			self.__addColor( slice, background=background )
		else: raise ColorError( 'Background color \'%s\' you can\'t use' % background )
	...
	def __slice( self, slice ):
		return slice or self.__defaultSlice()

	def __defaultSlice( self ):
		return slice( 0, MAXINT )

	def __isColor( self, color ):
		if self.__colors.has_key( color ):
			return True
		return False

	def __isBackground( self, color ):
		if self.__backgroundColors.has_key( color ):
			return True
		return False
